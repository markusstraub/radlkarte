...
- `leisure`=`swimming_pool`: Badestelle (bathing spot / swimming location)
- `priority`=`0` | `1` | `2` (optional): how prominently the point is drawn — `0` shows it from low zoom levels at full size, `2` only when zoomed in close. If absent, the point is treated as priority `1`; unreviewed points are highlighted in JOSM by the MapCSS style
...
The style highlights problems while you edit:
