...
6. Prepare (minify, add bbox,..) the .geojson with [prepare_geojson.py](data/prepare_geojson.py)
   - with the [development tools](#development): `npm run geojson -- [path to GeoJSON file]`
...
> [!NOTE]
> npm needs `--` before arguments meant for the script itself, so `npm run geojson -- data/radlkarte-wien.geojson`. Without it npm swallows the path.
